...
colors = {"Isotherme 0° pour l'année la plus froide sur la période": "blue", "Isotherme 0° moyen sur la période": "orange", "Isotherme 0° pour l'année la plus chaude sur la période": "red"}

for periode, stats in isothermes.items():
    plt.figure(figsize=(30, 12))

    plt.fill_between(x, df['Altitude_Sommet'], color='lightgrey', alpha=0.5)

    # Tracer la crête (sommets reliés par une ligne)
    plt.plot(x, df['Altitude_Sommet'], color='grey', marker='o', label='Sommet', linewidth=2)
    ...
    plt.scatter(x[mask_stations], df.loc[mask_stations, 'Altitude_basse_station de ski'],
                color='green', marker='o', s=100, label='Basse station de ski')
    plt.scatter(x[mask_stations], df.loc[mask_stations, 'Altitude_haute_station de ski'],
                color='green', marker='o', s=100, label='Haute station de ski')
    ...
    for xi, yb, yh in zip(x[mask_stations], df.loc[mask_stations, 'Altitude_basse_station de ski'],
        ...
        plt.plot([xi, xi], [yb, yh], color='green', linestyle='-', linewidth=3)
    ...
    for key in ["Isotherme 0° pour l'année la plus froide sur la période", "Isotherme 0° moyen sur la période", "Isotherme 0° pour l'année la plus chaude sur la période"]:
        plt.axhline(y=stats[key], color=colors[key], linestyle='-', linewidth=1, label=f'{key} {periode}')
...
images = [
    os.path.join(image_folder, "profile_0C_2020_2040.png"),
